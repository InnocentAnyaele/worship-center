...
export default function NavBar() {
  ...
  return (
    ...
    <nav
      className={`flex flex-row px-6 w-[100%] lg:items-center flex-wrap bg-[#99DAFF] fixed top-0`}
    >
      ...
      <div
        className={`${
          !active ? "" : "hidden"
        } w-full lg:w-auto md:w-auto text-[#B2B2B2] order-last lg:order-none md:order-none flex-wrap flex flex-col md:flex-row items-center`}
      >
        {navItems.map((item, index) => (
          <div
            key={index}
            className={`${
              pathname === item.path ? "p3 bg-[#D9D9D9] rounded" : "invinsible"
            }`}
          >
            <Link href={item.path} className="m-3">
