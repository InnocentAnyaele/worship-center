...
export default function Project() {
      ...
      const snapshots = await getDocs(contributionRefQuery).then(
        (snapshots) => {
          const docs = snapshots.docs.map((doc) => {
            ...
            let contributionFullDate =
              contributionMonth + " " + contributionYear;

            if (contributionFullDate in monthSum) {
              monthSum[contributionFullDate] =
  ...
  return (
    <div className="flex m-3 flex-col justify-center py-10 md:px-40 lg:px-40 flex-wrap w-[100%]">
      ...
      <button
        className="bg-[#8B7E74] px-2 h-10 rounded w-40"
        onClick={() => setIsAddContributionOpen(true)}
      >
        <span className="text-white text-sm">Add Contribution</span>
      </button>
